...
bubble_sort:
                                ; accepts array size in ecx, array
                                ; ptr in edx
	push ebx					; save regs
	push esi
	push edi
	...
	lea	eax, [edx + ecx*4 - 4]	; pointer to the last element =
	                            ; array ptr + (count - 1 ) *
	                            ; pointer_size (pointer size is 4
	                            ; bytes)
	mov	[last], eax

.outer_loop:

    mov	esi, ebx         ; ptr to the first array element
...
.dontswap:
    ...
    add	esi, 4			                ; move on to the next
                                        ; array element
    cmp	[last], esi		                ; don't sort beyond the
                                        ; "last element to sort"
    jnz	.inner_loop
    sub	dword [last], 4		            ; move pointer to the
                                        ; "last element to sort"
                                        ; forward because the rest
                                        ; is sorted already
